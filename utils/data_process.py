...
def convert_labelregion(labelvolum):
    target_label = np.zeros_like(labelvolum)
    target_label[np.where(labelvolum == 1)] = 1

    target_label[np.where(labelvolum == 3)] = 1
    print(np.unique(target_label))
    target_label[np.where(labelvolum == 1)] = 2
    target_label[np.where(labelvolum == 2)] = 2
    target_label[np.where(labelvolum == 3)] = 2
    target_label[np.where(labelvolum > 0)] = 2
    target_label[np.where(labelvolum == 3)] = 3
...
def itensity_normalize_one_volume(volume):
    ...
    """
    min_val = volume.min()
    pixels = volume[volume > min_val]
    mean = pixels.mean()
    std  = pixels.std()
    out = (volume - mean)/std
    out_random = np.random.normal(min_val, 1, size = volume.shape)
    out[volume == min_val] = out_random[volume == min_val]
    return out
...
def normalize_one_volume(volume):
    ...
    rang = max_val - min_val * 1.0
    volume = (volume - min_val) * 1.0 / rang
    return volume
...
def crop_ND_volume_with_bounding_box(volume, min_idx, max_idx):
    ...
    if(dim == 2):
        ...
        # print(range(min_idx[0], max_idx[0] + 1))
        # print(volume.shape)
        output = volume[np.ix_(range(min_idx[0], max_idx[0] + 1),
                               range(min_idx[1], max_idx[1] + 1),
                               range(min_idx[2], max_idx[2] + 1),
...
def get_largest_two_component(img, print_info = False, threshold = None):
    ...
    if(len(sizes) == 1):
        ...
        if(threshold):
            ...
            component1 = np.zeros_like(labeled_array)
            component2 = np.zeros_like(labeled_array)
            for lab in max_label1:
                component1 += labeled_array == lab 
                component1 = component1 > 0
            for lab in max_label2:
                component2 += labeled_array == lab 
                component2 = component2 > 0
            # print(labeled_array, max_label1, component1)
